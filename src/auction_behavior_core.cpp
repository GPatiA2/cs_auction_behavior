...
bool AuctionBehaviorCore::configure()
{
  ...
  std::string plugin_name = parent_->get_parameter("plugin_name").as_string() + "::Plugin";
  parent_->declare_parameter("state_component", std::vector<std::string>{});
  ...
  try {
    ...
    RCLCPP_ERROR(parent_->get_logger(), "Failed to load auction plugin '%s': %s",
  ...
  ca_client_.register_module<as2_msgs::msg::StartAuction>(
    ...
    [this](const as2_msgs::msg::StartAuction & msg, const std::string & agent_id) {
      ...
      // Participant fires a self-goal so on_run() ticks
...
bool AuctionBehaviorCore::activate()
{
  return true;
}
...
void AuctionBehaviorCore::handle_accepted(std::shared_ptr<GoalHandleT> goal_handle)
{
  ...
  // Start periodic on_run() ticker
  float run_freq = 10.0f;
  if (parent_->has_parameter("run_frequency")) {
    run_freq = parent_->get_parameter("run_frequency").as_double();
  }
  ...
  if (success) {
    ...
    for (size_t i = 0; i < result_.elements.size(); ++i) {
      RCLCPP_INFO(parent_->get_logger(), "  %s -> %s",
        strip_ns(result_.winners[i]).c_str(), result_.elements[i].name.c_str());
    }
  }
  goal_handle_.reset();
}
...
bool AuctionBehaviorCore::load_item_plugin(const std::string & item_type)
{
  ...
  try {
    ...
    RCLCPP_ERROR(parent_->get_logger(), "Failed to load item plugin '%s': %s",
